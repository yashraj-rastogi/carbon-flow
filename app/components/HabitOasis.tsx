...
export default function HabitOasis({ strength }: HabitOasisProps) {
  // Scaling factors based on habit strength
  const particleCount = strength * 3;
  const coreScale = 0.8 + (strength / 10) * 0.4;
  const glowIntensity = strength * 2.5;

  return (
    <div className="relative w-full h-[360px] md:h-[400px] flex items-center justify-center overflow-hidden rounded-2xl border border-emerald-500/20 bg-emerald-950/10 shadow-[inset_0_0_50px_rgba(16,185,129,0.05)]">
      {/* Background Cyberpunk Grid Grid */}
      <div className="absolute inset-0 cyber-grid-oasis opacity-60" />
      ...
      >
        ...
        >
          ...
          <span className="text-emerald-400 text-xs font-bold uppercase tracking-widest mb-1">Oasis Core</span>
          ...
          <span className="text-emerald-300/80 text-[10px] uppercase font-semibold tracking-wider mt-2">
            Habit Strength
          </span>
        </div>
      </motion.div>
      ...
      <div className="absolute bottom-0 w-full h-1/3 bg-gradient-to-t from-emerald-950/40 to-transparent flex items-end justify-center pointer-events-none">
        <div className="w-[80%] h-[2px] bg-gradient-to-r from-transparent via-emerald-500/40 to-transparent filter blur-xs" />
      </div>

      <div className="absolute top-4 left-4 flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-950/60 border border-emerald-500/20">
        <div className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse" />
        <span className="text-[11px] font-bold text-emerald-300 uppercase tracking-widest">Flora System Stable</span>
      </div>
    </div>
  );
}
